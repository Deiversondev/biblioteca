                ...
                if (filtro != null)
                {
                    switch (filtro.TipoFiltro)
                    {
                        ...
                        case "Livro":
                            ...
                            List<Livro> LivroFiltrados = bc.Livros.Where(l => l.Titulo.Contains(filtro.Filtro)).ToList();

                            List<int> LivrosIds = new List<int>();
                            for (int i = 0; i < LivroFiltrados.Count; i++)
                            {
                                LivrosIds.Add(LivroFiltrados[i].Id);
                            }
